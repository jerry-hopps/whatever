...
public class UserController {
	...
	@RequestMapping(value="/register/{id}/{token}.html",method=RequestMethod.GET)
	public ModelAndView register(@PathVariable("id") Integer id, @PathVariable("token") String token){
		ModelAndView mav = new ModelAndView("register");
		
		byte[] inputData = token.trim().getBytes();
		String key = this.userService.findUserById(id).getPassword().trim();
		try {
			String email = new String(DESCoder.decrypt(inputData, key));
			mav.addObject("email", email);
		} catch (Exception e) {
			mav.setViewName("user/invalidtoken");
		}
